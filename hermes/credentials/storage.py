...
def init_credentials() -> Path:
    try:
        ...
        raise CredentialStorageError(
            f"Could not initialize credential store: {exc}"
        ) from exc
...
def load_credentials() -> dict[str, Any]:
    if not CREDENTIALS_FILE.exists():
        raise CredentialStorageError(
            "Hermes credentials are not initialized. "
            "Run `hermes cred init` first."
        )
    ...
    try:
        ...
        raise CredentialStorageError(
            f"Invalid credentials file: {CREDENTIALS_FILE}"
        ) from exc
    except OSError as exc:
        raise CredentialStorageError(
