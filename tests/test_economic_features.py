...
class TestEconomicFeatures:
    ...
    def mock_wb(self, eco, sample_wb_df):
        eco.wb = MagicMock()
        eco.wb.fetch = MagicMock(return_value=sample_wb_df)
        return eco.wb

    @pytest.fixture
    def mock_wb_cpi(self, eco, sample_wb_cpi_df):
        eco.wb = MagicMock()
        eco.wb.fetch = MagicMock(return_value=sample_wb_cpi_df)
        return eco.wb

    @pytest.fixture
    def mock_imf(self, eco, sample_imf_df):
        eco.imf = MagicMock()
        eco.imf.fetch = MagicMock(return_value=sample_imf_df)
        return eco.imf
    ...
    def test_gdp_growth_yoy_ML(self, eco):
        eco.wb = MagicMock()
        eco.wb.fetch = MagicMock(
    ...
    def test_gdp_growth_yoy_empty(self, eco):
        eco.wb = MagicMock()
        eco.wb.fetch = MagicMock(return_value=pl.DataFrame())
