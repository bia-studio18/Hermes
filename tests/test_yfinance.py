...
class TestYfinance:
    def test_fetch_eps_estimate(self):
        ...
        with patch("hermes.connectors.yfinance.connector.yf.Ticker", return_value=mock_ticker):
            result = yf._fetch(endpoint="eps_estimate", symbol="AAPL")
            assert result is not None
            assert "avg" in result

    def test_fetch_revenue_estimate(self):
        yf = Yfinance(cache=None)
        ...
        with patch("hermes.connectors.yfinance.connector.yf.Ticker", return_value=mock_ticker):
            result = yf._fetch(endpoint="revenue_estimate", symbol="AAPL")
            assert result is not None
            assert "avg" in result
    ...
    def test_fetch_earnings_history(self):
        ...
        with patch("hermes.connectors.yfinance.connector.yf.Ticker", return_value=mock_ticker):
            result = yf._fetch(endpoint="earnings_history", symbol="AAPL")
            assert result is not None
            assert "surprisePercent" in result

    def test_fetch_unsupported_endpoint(self):
        yf = Yfinance(cache=None)
        mock_ticker = MagicMock()
        mock_ticker.info = {}
        ...
        with patch("hermes.connectors.yfinance.connector.yf.Ticker", return_value=mock_ticker):
            with pytest.raises(ValueError, match="Unsupported endpoint"):
                yf._fetch(endpoint="bad_endpoint", symbol="AAPL")

    def test_fetch_none_data_returns_none(self):
        yf = Yfinance(cache=None)
        mock_ticker = MagicMock()
    ...
    def test_fetch_empty_df_returns_none(self):
        yf = Yfinance(cache=None)
